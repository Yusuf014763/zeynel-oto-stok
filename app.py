  ...
  with tab_servis:
    ...
    with st.form("yeni_kayit_formu", clear_on_submit=True):
      ...
      with col_b:
        ...
        yapilan_islem = st.text_area(
            "Yapılan İşlemler (Örn: Triger değişimi, Bakım)"
        )
        tutar = st.number_input("Toplam Tutar (TL)", min_value=0.0, step=50.0)
        odeme_durumu = st.selectbox(
            "Ödeme Durumu", ["Ödendi", "Bekliyor", "Kısmi Ödeme"]
        )
      ...
      if submit_button:
        if plaka and yapilan_islem:
          ...
          st.session_state.servis_df = pd.concat(
              [st.session_state.servis_df, pd.DataFrame([yeni_servis])],
              ignore_index=True,
          )
          save_servis(st.session_state.servis_df)
          st.success(
              f"{plaka} plakalı araç için müşteri/servis kaydı başarıyla eklendi."
          )
        else:
